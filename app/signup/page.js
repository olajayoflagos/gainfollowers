...
export default function SignupPage() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      await fetch('/api/email/welcome', { method: 'POST', body: JSON.stringify({ email, name }), headers: { 'Content-Type':'application/json' } });
      await sendEmailVerification(cred.user);
      setMsg({ text: 'Account created. Please verify your email.', type: 'success' });
      setTimeout(()=>router.push('/login'), 1200);
    } catch (e) {
      const t = e.code === 'auth/email-already-in-use'
        ? 'This email already has an account. Log in or use Google; or reset your password.'
        : e.message;
