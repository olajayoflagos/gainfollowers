...
/** tolerate Firestore Timestamp, ISO string, number(ms), or undefined */
function toIso(val) {
  try {
    if (!val) return '';
...
export default function Dashboard() {
  ...
  const serviceNameById = useMemo(() => {
    const m = new Map();
    for (const s of services || []) m.set(String(s.service), s.name);
    return m;
  }, [services]);
  ...
  useEffect(() => {
    ...
    const unsub = onAuthStateChanged(auth, async (u) => {
      ...
      try {
        ...
        setToast({ text: 'Failed to load initial data.', type: 'error' });
      } finally {
        setBooting(false);
      }

      // manual loads after boot
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (data?.index_required) {
          setToast({ text: 'Firestore index required (uid + createdAt desc).', type: 'error' });
        }
        return;
      }
      ...
      setToast({ text: 'Could not load orders.', type: 'error' });
    } finally {
  ...
  async function refreshHistory() {
    try {
      ...
      const [txRes, wRes] = await Promise.all([
        fetch('/api/wallet/history', { headers: { Authorization: `Bearer ${token}` }, cache: 'no-store' }),
        fetch('/api/user/me', { headers: { Authorization: `Bearer ${token}` }, cache: 'no-store' }),
      ]);
      ...
      setToast({ text: 'Could not load transactions.', type: 'error' });
    } finally {
      setLoadingTx(false);
    }
  }
  ...
  /* ---------- auth gates ---------- */
  if (booting) return (<><NavBar /><Spinner label="Loading Dashboard…" /></>);
  if (!user) {
    return (
      <>
        <NavBar />
        ...
        <section className="card p-5 md:p-6 rounded-xl border border-gray-200 dark:border-gray-800 shadow-sm">
          <div className="flex items-center justify-between gap-3 flex-wrap">
            ...
            <button
              ...
              onClick={() => { setOrder({ service: '', link: '', quantity: '' }); setSearch(''); }}
            >
              Clear form
            </button>
          </div>
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-3">
            ...
            <div className="md:col-span-3 flex items-center justify-between flex-wrap gap-3">
              <div className="text-sm text-gray-500">
                Estimated price:&nbsp;
                <span className="font-semibold text-gray-800 dark:text-gray-100">{NGN(estimate)}</span>
              </div>
              <button
        ...
        <section className="card p-5 md:p-6 rounded-xl border border-gray-200 dark:border-gray-800 shadow-sm">
          <div className="flex items-center justify-between gap-3 flex-wrap">
            ...
            <div className="flex gap-2">
              ...
              {nextCursor && (
                <button className="btn-outline" onClick={loadMoreOrders} disabled={loadingOrders}>
                ...
                <tbody>
                  {orders.map((o) => (
                    <tr key={o.id} className="border-b border-gray-100 dark:border-gray-900">
                      ...
                      <td className="py-2 pr-4">{NGN(o.priceNGN)}</td>
                      <td className="py-2 pr-4 capitalize">{o.status}</td>
                      <td className="py-2 pr-4">{humanDate(o.createdAt)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
                  ...
                  {transactions.map((t) => (
                    <tr key={t.id} className="border-b border-gray-100 dark:border-gray-900">
                      ...
                      <td className={`py-2 pr-4 ${t.type === 'credit' ? 'text-emerald-600' : 'text-red-600'}`}>
                        {t.type === 'credit' ? '+' : '-'} {NGN(t.amountNGN)}
                      </td>
                      <td className="py-2 pr-4">{t.reference || t.orderId || '-'}</td>
                      <td className="py-2 pr-4">{humanDate(t.createdAt)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
