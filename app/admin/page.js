...
const AdminOrdersTable = dynamic(
  () => import('../components/AdminOrdersTable'),
  { ssr: false, loading: () => <Spinner label="Loading orders…" /> }
);
const AdminUsersList = dynamic(
  () => import('../components/AdminUsersList'),
  { ssr: false, loading: () => <Spinner label="Loading users…" /> }
);

// Tiny toast
function Toast({ text, type = 'info' }) {
  if (!text) return null;
  const base = 'fixed z-40 bottom-4 left-1/2 -translate-x-1/2 rounded-lg px-4 py-2 shadow-lg';
...
export default function AdminPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (!token) { setErr('Please log in as admin'); setOk(false); return; }
  ...
  const filteredOrders = useMemo(() => {
    ...
    return (orders || []).filter(o => {
      ...
      const statusOk = !st || (o.status && String(o.status).toLowerCase() === st);
      return hit && statusOk;
    });
  ...
  const stats = useMemo(() => {
    ...
    const totalNGN = filteredOrders.reduce((a, b) => a + Number(b.priceNGN || 0), 0);
    return { n, totalNGN };
  }, [filteredOrders]);
  ...
  const adjust = async () => {
    try {
      ...
      if (!v || !Number.isFinite(v)) return setToast({ text: 'Enter a valid amount (₦)', type: 'error' });
      const token = await getToken();
    ...
    return (
      <>
        ...
        <div className="container py-6">
          <div className="card p-5 text-red-600">
            {err || 'Not admin'}
          </div>
        </div>
      </>
      ...
      <div className="container py-6 space-y-6">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <div className="hidden md:flex items-center gap-4 text-sm text-gray-500">
                <div>Orders: <span className="font-medium text-gray-800 dark:text-gray-100">{stats.n}</span></div>
                <div>Revenue: <span className="font-medium text-gray-800 dark:text-gray-100">₦{Number(stats.totalNGN).toLocaleString()}</span></div>
                <div>Users: <span className="font-medium text-gray-800 dark:text-gray-100">{users.length}</span></div>
              </div>
            </div>

            <div className="flex flex-wrap items-center gap-2">
              <input
                className="h-9 w-56 rounded-md border border-gray-300 bg-transparent px-3 text-sm dark:border-gray-700"
                placeholder="Search orders (id/service/email)…"
                value={search}
                onChange={(e) => setSearch(e.target.value)}
              />
              <select
                className="h-9 rounded-md border border-gray-300 bg-transparent px-3 text-sm dark:border-gray-700"
                value={status}
              ...
              <button className="btn-outline h-9" onClick={fetchData}>Refresh</button>
              <button className="btn-outline h-9" onClick={exportCSV}>Export CSV</button>
        ...
        <div className="grid gap-6 md:grid-cols-3">
          ...
          <section className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
            ...
            <AdminUsersList users={users.filter(u => {
              const q = (search || '').toLowerCase();
              return !q
            ...
            <div className="mt-6 space-y-2">
              ...
              <div className="flex gap-2">
                ...
                <button className="btn-outline" onClick={() => { setUidAdjust(''); setDelta(''); }}>Clear</button>
              </div>
